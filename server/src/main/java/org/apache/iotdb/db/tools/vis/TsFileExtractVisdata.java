 ...
 * <p>
 * All input TsFiles should be sealed. The handle of unsealed TsFile is left as future work when in
 * need.
 * <p>
 ...
 * <p>
 * NOTE: There is no version info stored in TsFileV3/IoTDBV0.12 any more. As a result,
 * TsFileExtractVisdata (1) will not extract version info as it does to TsFileV2/IoTDBV0.11
 * and (2) will not use reader.selfCheck to get ChunkMetadata, which will be fetched by reading
 * metadata at the end of the TsFile instead. That means, the input TsFiles should be sealed.
 * The handle of unsealed TsFile is left as future work when in need.
 */
        ...
        for (String f : filelist) {
          ...
          try (TsFileSequenceReader reader = new TsFileSequenceReader(f)) {
            ...
            for (String device : reader.getAllDevices()) {
              Map<String, List<ChunkMetadata>> seriesMetaData = reader
