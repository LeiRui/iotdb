...
public class ChunkReader implements IChunkReader {
  ...
  private PageReader constructPageReaderForNextPage(PageHeader pageHeader) throws IOException {
    ByteBuffer pageData;
    if (!TSFileDescriptor.getInstance().getConfig().isUncompressedSave()
        || chunkHeader.getCompressionType() != CompressionType.UNCOMPRESSED) {
      ...
      // TODO get a slice from chunkDataBuffer, and position chunkDataBuffer
